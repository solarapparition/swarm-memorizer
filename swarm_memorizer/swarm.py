...
class ReasoningGenerator:
    ...
    def generate_learning_reasoning(self) -> str:
        ...
        - EVENTS LOG: a log of events that have occurred during the execution of the task. This can include status updates for subtasks, messages from the {MAIN_TASK_OWNER}, and the orchestrator's own previous thoughts/decisions.
        - {SUBTASK} LIST: a list of {SUBTASK}s that were done (or attempted) for the {MAIN_TASK}. For each {SUBTASK}, the following information is available:
          - {NAME}: a high-level summary of what the {SUBTASK} is.
          ...
          - {ARTIFACT}s: a list of {ARTIFACT}s that were generated by the {EXECUTOR} of the {SUBTASK} while completing it.



        # ....
        # > executor_id here is actually blueprint id
        ...
        # > add concept for executor blueprint id
        # > remove name of executor in delegation—to focus attention on description
        # > instead of printing out executor id, print out executor name
        # > in orchestrator status, subtask printout needs to say "summary" instead of Name
    ...
    print(self.base_info(ActionModeName.DEFAULT))
    # orchestrator_concepts=ORCHESTRATOR_CONCEPTS,
...
class Orchestrator:
    ...
    def determine_knowledge(self) -> str:
        ...
        reasoning = self.generate_learning_reasoning()
        breakpoint()
